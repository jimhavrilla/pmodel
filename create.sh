...
bedtools intersect -a <(cat $DATA/doms-any-coverage.bed | tr -s "\t" " " | cut -d " " -f 1-45 | tr -s " " "\t") -b <(awk '{if ($4>=5) print}' $DATA/coverage.bed) -wa -wb -sorted \
...
awk '{split($15,trans,","); for (i in trans) print $1,$2,$3,$13,$25,trans[i],$43}' $DATA/doms.bed | sort -k6,6 | tr -s " " "\t"  > $DATA/foo3
python nodom.py $DATA/foo2 $DATA/foo3 > $DATA/nodoms-any-coverage.bed; rm $DATA/foo2 $DATA/foo3
