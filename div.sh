...
#$2=database - $DATA/varfilter.db,$3=omimfile - $DATA/omim.txt,$4=greater/less/range,$5=leftbound,$6=rightbound

bash query.sh $DATA/varfilter.db '%' $MOD $MAF1 $MAF2 | sort -t '|' -k7,7 -k1,1 -k2,2 -k3,3 | python formatvar.py > $DATA/allint2uniqfilter.$MOD.$MAF1-$MAF2.bed
...
case "$1" in
	"-u"|"--uniqid")
		...
		awk 'NR==FNR{a[$2]=$3} NR!=FNR{if ($1 in a) print $0,a[$1]; else print $0,0}' $DATA/human_pfam.counts $DATA/uniqtablefilter.$MOD.$MAF1-$MAF2.txt > $DATA/foo2; mv $DATA/foo2 $DATA/uniqtablefilter.$MOD.$MAF1-$MAF2.txt

		awk 'NR==FNR{a[$2]=$3} NR!=FNR{if ($1 in a) print $0,a[$1]; else print $0,0}' $DATA/human_pfam.counts $DATA/nodomtablefilter.$MOD.$MAF1-$MAF2.txt > $DATA/foo2; mv $DATA/foo2 $DATA/nodomtablefilter.$MOD.$MAF1-$MAF2.txt

		cat $DATA/nodomtablefilter.$MOD.$MAF1-$MAF2.txt $DATA/uniqtablefilter.$MOD.$MAF1-$MAF2.txt | sort -k11,11nr > $DATA/regionstable.$MOD.$MAF1-$MAF2.txt
