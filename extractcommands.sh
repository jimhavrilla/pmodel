...
#sudo perl $SOFTWARE/ensembl-tools-release-76/scripts/variant_effect_predictor/variant_effect_predictor.pl -i $DATA/ExAC.r0.2.sites.vep.vcf --cache --sift b --polyphen b --symbol --numbers --biotype --total_length -o $DATA/VEPEXAC.vcf --vcf --fields Consequence,Codons,Amino_acids,Gene,SYMBOL,Feature,EXON,PolyPhen,SIFT,Protein_position,BIOTYPE --force_overwrite --port 3337 --dir ~/.vep/ --compress "gunzip -c"
...
bedtools intersect -a <(sort -k1,1 -k2,2n -k3,3n $DATA/alluniq.bed | tr -s " " "\t" | cut -f 1,2,3,11,13,45 ) -b $DATA/VEPEXAC.vcf -sorted -wb | cut -f 1,2,3,4,5,6,10,11,14 | python var.py -d > $DATA/uniqint.bed
bedtools intersect -a <(sort -k1,1 -k2,2n -k3,3n $DATA/nodom.bed | tr -s " " "\t" | cut -f 1,2,3,12,24 ) -b $DATA/VEPEXAC.vcf -sorted -wb | cut -f 1,2,3,4,5,9,10,13 | python var.py -n > $DATA/nodomint.bed
...
cat $DATA/uniqint.bed $DATA/nodomint.bed | sort -k1,1 -k2,2n | cat <(printf "#chr,start,end,ref,alt,pfamA_id,uniqid,gene_symbol,maf,impact,codons,amino_acids,gene_id_csq,gene_symbol_csq,transcript_id_csq,exon_number_csq,polyphen,sift,protein_position,biotype\n") - > $DATA/allint.bed
